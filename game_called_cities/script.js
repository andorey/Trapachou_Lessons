...
field.addEventListener('keypress', function (event) {
    ...
    if(event.key === 'Enter') {
        ...
        let lastLetter = letter.slice(letter.length - 1) === 'ь' || letter.slice(letter.length - 1) === 'ы' ? letter.slice(letter.length - 2, letter.length - 1) : letter.slice(letter.length - 1);

        if ( arrCities.includes(letter) && (lastChapter === firstChapter || lastChapter === 0)) {
            arrCities = arrCities.filter(el => el !== letter);

            const cityNames = arrCities.filter(el => el.toLowerCase()[0] === lastLetter)

            if (cityNames.length < 1){
                temp = 'ВЫ ПОБЕДИЛИ!!!'
                message.classList.add('error')
            }else{
                temp = arrCities.splice(arrCities.indexOf(cityNames[0]), 1);
            }
        ...
        }else if ( lastChapter !== firstChapter ){
            message.classList.add('error');
            message.innerText = 'Ваша первая буква должна соответствовать последней предложенной компютером \n Попробуйте еще раз'

        }else{
            message.classList.add('error');
            message.innerText = 'Такого города не существует \n либо он был упомянут ранее!\n Ввидите другое название города'
        }
    }
})
